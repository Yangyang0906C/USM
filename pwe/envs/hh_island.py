...
class Island:
    ...
    def initialization(self, args):
        ...
        self.observation_space = gym.spaces.Box(low=-1, high=1,
                                                shape=[(args.size * 2 + self.agent_max_power) * self.n_agent +
                                                       (args.size * 2) * self.n_wolf + self.n_landmark + self.n_wolf * int(
                                                    self.is_full_obs)])
    ...
    def step(self, action_n):
        ...
        return return_rew, return_done, info
    ...
    def obs(self, i):
        wolf = np.concatenate([np.concatenate([self.eye[wolf[0]], self.eye[wolf[1]]], axis=0)
                               for i, wolf in enumerate(self.wolf_n)], axis=0)
        a_state = np.concatenate([np.concatenate([self.eye[self.state_n[i][0]], self.eye[self.state_n[i][1]],
                                                  self.agent_power_eye[self.agent_power[i]]], axis=0),
                                  np.concatenate([self.eye[self.state_n[1 - i][0]], self.eye[self.state_n[1 - i][1]],
                                                  self.agent_power_eye[self.agent_power[i]]], axis=0)])
    ...
    def get_state_mini(self):
        a = np.concatenate([np.concatenate([state, [self.agent_power[i]]]) for i, state in enumerate(self.state_n)])
        w = np.concatenate([self.wolf_n[0], [self.wolf_power[0]]])
        return np.concatenate([a, w, [np.array(self.landmark_visited).sum()]])
    ...
    def get_avail_actions(self):
        ...
        for agent_id in range(self.n_agent):
            avail_action = [1 for i in range(self.n_action)]
            avail_actions.append(avail_action)
        return avail_actions


    def get_env_info(self):
        env_info = {"state_shape": (self.size * 2 + self.agent_max_power) * self.n_agent +
                                   (self.size * 2) * self.n_wolf + self.n_landmark + self.n_wolf,
                    "mini_state_shape": 10,
                    "obs_shape": (self.size * 2 + self.agent_max_power) * self.n_agent +
                                 (self.size * 2) * self.n_wolf + self.n_landmark + self.n_wolf,
                    "n_actions": self.n_action,
                    "n_agents": self.n_agent,
                    "episode_limit": self.H,
                    "nvec": [self.agent_max_power for i in range(10)]}
        return env_info
