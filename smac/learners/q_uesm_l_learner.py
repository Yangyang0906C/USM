...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, sub_mask):
        ...
        for t in range(batch.max_seq_length - 1):
            rew_int = th.abs(state[:, t, :] - state[:, t + 1, :]) * sub_mask
            rew_int = th.sum(rew_int, dim=1).unsqueeze(-1)
            rewards_tmp[:, t] = rewards[:, t] + (rew_int / sum(sub_mask)) * self.args.rwd_int_lambda
        ...
        with th.no_grad():
            ...
            if self.args.td_lambda == True:
                targets = build_td_lambda_targets(rewards_tmp, terminated, mask, target_max_qvals,
                                                  self.args.gamma, self.args.lambdaa)
            else:
        ...
        per_rwd = (rewards_tmp * mask).sum(1)
        return per_rwd
